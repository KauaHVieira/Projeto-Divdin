Public Class quitar_despesa_grupo
    Private Sub quitar_despesa_grupo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        conecta_banco_mysql()
        ...
        sql = $"SELECT 
            ...
            WHERE (d.pago_por = {token} OR dp.usuario_id = {token} OR dd.usuario_id = {token})
              AND d.id_despesa = {id_despesa} and d.grupo_id = {id_gp}
    ...
    Private Sub btn_registrar_despesa_Click(sender As Object, e As EventArgs) Handles btn_registrar_despesa.Click
        ...
        If novo_valor_total <= 0 Then
            sql = $"delete from despesas where id_despesa='{id_despesa}' and grupo_id = '{id_gp}'"
            db.Execute(sql)
        Else
            sql = $"update despesas set valor_total='{novo_valor_total}' where id_despesa='{id_despesa}' and grupo_id ='{id_gp}'"
            db.Execute(sql)
        End If
